...
Page({
  ...
  onLoad: function (options) {
    ...
    if(datalist.indexOf('recruitmentProgress:notSuitable')==-1){
      this.setData({
        userReject:true
      })
    }
    if(datalist.indexOf('recruitmentProgress:invitationInterview')==-1){
      this.setData({
        userEntry:true
      })
    }
  ...
  getEntryList(){
    let that = this;
    let _type = that.data.tabbarNum==1?'newResume':'resumeIsViewed'
  ...
  enterReject(e) {
    ...
    console.log(applyid)
    wx.navigateTo({
      url: `../../pages/jobmsg/jobmsg?info=notSuitable&data=${JSON.stringify(applyid)}&addr=${JSON.stringify([])}`
    })
  },
  enterResolve(e) {
    ...
    let addrdata = { Addr: data.fmtAddr,addrid: data.interViewAddrId, addrBvid: data.interviewBvAddrId }
    wx.setStorage({
      key:"jobcomname",                   
      data:data.jobName
    })
    wx.navigateTo({
      url: `../../pages/jobmsg/jobmsg?info=invitationInterview&data=${JSON.stringify(applyid)}&addr=${JSON.stringify(addrdata)}&companyId=${data.companyId}`
    })
  ...
  gotoJoblist(e){
    ...
    wx.navigateTo({
      url: `../../pages/joblist/joblist?info=${e.currentTarget.dataset.info}`
    })
  },
})