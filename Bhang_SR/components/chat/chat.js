...
Component({
	...
	methods: {
		...
		onMakeVideoCall(data){   
			console.log(data)
			//  demo里面去会议群里选取人员，这里直接跳过，直接尝试发起视频 
			this.setData({
				showEmediaInvite: false,
		...
		onCreateConfrSuccess(data){
			...
			getApp().globalData.confrId = data.detail.confrId
   			this.sendInviteMsg(data.detail.confrId,data.detail.ticket)
		},

		sendInviteMsg(confrId,ticket){
			console.log(this.properties.username)
			let userId = this.properties.username.your
			let id = WebIM.conn.getUniqueId();
			let msg = new WebIM.message('txt', id);
			msg.set({
				msg: wx.getStorageSync("imUserInfo").imUsername + ' invite you to video call',
				from: wx.getStorageSync("imUserInfo").imUsername,
				to: userId,
				roomType: false,
				chatType: 'chat',
				ext: {
					msg_extension: JSON.stringify({
						inviter: wx.getStorageSync("imUserInfo").imUsername,
						group_id: ''
					}),
					password: '',
				...
				success(id, serverMsgId){
					console.log('发送邀请消息成功 to: '+userId)
