	...
	methods: {
		...
		sendMessage(){
			...
			new Promise((resolve,rejuect)=>{
				getUserByImUsername({idStr:wx.getStorageSync("imUserInfo").imUsername}).then(res=>{
					console.log(res)
					resolve(res.body[0])
				})
			}).then(nickdata=>{
				...
				msg.set({
					...
					roomType: false,
					ext: {userName:nickdata.nickname, em_robot_message: false, userPicMy: nickdata.avatar},   
