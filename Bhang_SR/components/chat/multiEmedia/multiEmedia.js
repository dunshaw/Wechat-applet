...
Component({
	...
	lifetimes: {
	    attached: function() {
			...
			wx.emedia.mgr.onMemberJoined = function(mem){
				...
				console.log("++++++++++ member", mem,identityName)
				// 如果是自己进入会议了，开始发布流
	...
	methods: {
		createConf(){
			...
			wx.emedia.mgr.createConference(11, '', false, false).then(function(data){
				console.log('成功', data)
				...
				me.triggerEvent('createConfrSuccess', {confrId: confrId, ticket:ticket, groupId: ''})
			})
		},
		...
		joinConf(data){
			...
			let me = this
			wx.emedia.mgr.joinConference(data.confrId, data.password).then(function(res){
					console.log('加入会议成功', res)

					me.setData({
						confrId: data.confrId
					})
					me.triggerEvent('createConfrSuccess', {confrId: data.confrId, groupId: ''})
				})
